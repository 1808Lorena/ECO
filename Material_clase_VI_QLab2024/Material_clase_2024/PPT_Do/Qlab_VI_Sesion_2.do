...
* Directorio de uso:
cd "D:\Trabajo\Docencia\PUCP\QLab\QLab-2024\Mod_Instrumental_variables_24\Material_clase_2024\Data"

* Log file:
log using "rep_results.log", replace
*se guardara en mi carpeta todo lo q hagas
...
**cada vez q llame a xfl sabra q llamo a esta lista
...
/* TABLE 2: FIRST STAGE */
*endogena con los instrumentos y otras de control

*5 columnas con opciones distintas donde x es explicativa y cambia qué controles incluir
...
dprobit any_prio gdp_g gdp_g_l $x_fl year, robust cluster(ccode)
	*metodologia para ver variables dependientes binarias xq era 1 con conflicto 0 sino. 
...
regress any_prio gdp_g gdp_g_l $x_fl $x_year (GPCP_g GPCP_g_l $x_fl $x_year), robust cluster(ccode)
	...
	*en parentesis pones variables q depeden d otras
...
ivreg2 any_prio $x_fl $x_year (gdp_g gdp_g_l=GPCP_g GPCP_g_l), robust cluster(ccode) 
		...
		*esta esta = dependen d estos instrumentos
